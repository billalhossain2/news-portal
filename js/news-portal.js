...
const displayCategories = (categories) => {
  ...
  categories.forEach(
    (category) =>
      (categoryUl.innerHTML += `<li onclick={loadCategoryNews('${category.category_id}')} class="cursor-pointer">${category.category_name}</li>`)
  );
};
                        ...
                        <p class="text-justify">${
                          details.length > 250
                            ? `<span>${details.slice(
                                0,
                                250
                              )} <button class="text-yellow-600">Read More...</button></span>`
                            : details
                        }</p>
                        <div class="flex lg:flex-row flex-col gap-5 justify-between mt-5 items-center">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p>${
                                      author.published_date || "Not found!"
                                    }</p>
                                </div>
                            </div>
                            <div>
